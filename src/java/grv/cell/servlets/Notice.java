...
public class Notice extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try 
        {
           ...
           String sql="Insert into notice(type,notice,notice_date) values(?,?,?)";
           PreparedStatement ps = con.prepareStatement(sql);
           ps.setString(1, type);
           ps.setString(2, notice);
           ps.setString(3, DateTime.sysDateTime());
           int row=ps.executeUpdate();
           out.println(row+" row inserted");
           if(row>0)
           {
                           
               request.setAttribute("status","Notice Submitted Successfully !!");
                
               RequestDispatcher rd=getServletContext().getRequestDispatcher("/NoticeWrite.jsp");
